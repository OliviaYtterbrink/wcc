...
exports.championsPlayed = function (req, res, next) {
    ...
    const limit = parseInt(req.query.limit);
    if (!summoner) throw {message: 'Invalid summoner name.', status: 400};
    Challenge.championsPlayed(dbUtils.getSession(req), summoner, skip, limit)
             .then(response => writeResponse(res, response))
             .catch(next);
}

exports.playedCount = function (req, res, next) {
  const summoner = req.params.summoner;
  if (!summoner) throw {message: 'Invalid summoner name.', status: 400};
  Challenge.championsPlayedCount(dbUtils.getSession(req), summoner)
           .then(response => writeResponse(res, response))
           .catch(next);
}
